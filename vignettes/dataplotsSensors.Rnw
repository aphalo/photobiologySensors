...
\title{\PBSen Version 0.1.3\\ Catalogue of Sensors}
...
<<setup-function>>=
sensor.plotter <- function(sensor.name, w.low=250.0, w.high=900.0, scaled="area"){
...
fig_energy <- ggplot(aes(x=w.length, y=response), data=e.spectrum.data) + xlim(w.low, w.high) +
  labs(x="Wavelength (nm)", y="Spectral response (energy)", title=sensor.name) + 
  geom_line()
fig_photon <- ggplot(aes(x=w.length, y=response), data=q.spectrum.data) + xlim(w.low, w.high) +
...
for (sensor in uvb.sensors) {
  sensor.plotter(sensor, w.low=200, w.high=500, scaled="area")
}
@
...
<<plot-ery>>=
uv_ery.sensors <- c("Berger_UV_erythemal", "TOCON_E2_erythemal", "KIPP_UVS_E_erythemal", 
